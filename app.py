...
def streamHandler(magnet_link):
    global webtorrentProc
    path = streamDirHandler()
    os.chdir(path)
    cmd0 = []
    cmd0.append("webtorrent")
    cmd0.append(magnet_link)
    if sys.platform.startswith("win32"):
        webtorrentProc = subprocess.Popen(
            cmd0, shell=True, stdout=subprocess.PIPE)
        print(webtorrentProc)
        #  while(True):
        #      print(webtorrentProc.output)
...
def api(name):
    ...
    for result in torr_results:
        if index < 50:
            if 'movie' in result['type'].lower() and result['nsfw'] == False:
                print(index, ". ", result['name'], "  ", result['size']," ",result['seeder'])
                index += 1
